...
class HandlerBoard:
    ...
    def walk(self, player: Player):
        ...
        if player.position < steps:
            print(
                f"- {player.strategy} completed a lap! They gained {config.PLAYER_MONEY_ROUND} in balance."
            )
    ...
    def buy_property(self, player: Player, property: Property):
        if self.decide_to_buy_property(player, property):
            ...
            print(
                f"- {player.strategy} buyed property {property.id} for {property.price} money."
            )
        else:
    ...
    def pay_rent(self, player: Player, property: Property):
        if player != property.owner:
            print(
                f"- {player.strategy} is paying rent of {property.rent_price} to Player {property.owner.strategy}."
            )
    ...
    def decide_to_buy_property(self, player: Player, property: Property):
        if player.money < property.price:
            print(
                f"- {player.strategy} does not have enough money to buy property {property.id}."
            )
            return False
